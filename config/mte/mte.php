...
class MySQLtabledit {
	...
	*/
	...
	####################
				...
				foreach ($rij AS $key => $value) {
					...
					if (in_array($key, $this->fields_in_list_view,true)) {
						...
						if ($key == $this->primary_key) {
							$buttons = "<td nowrap><a href='javascript:void(0)' onclick='del_confirm($value)' title='Delete {$this->show_text_listview[$key]} $value'><a href='?$query_string&mte_a=edit&id=$value' title='Edit {$this->show_text_listview[$key]} $value'><img src='$this->url_base/images/edit.png' class='icons' title='{$this->text['Edit']}'></a></td>";
		...
		# page content
		$this->content = "
			<div style='width: $this->width_editor;display:none;background:#454545;margin:0'>$seach_form</div>
